	...
	for r := 1; r < len(csvData); r++ {
		...
		for c := 0; c < len(csvData[r]); c++ {
			...
			case "int":
				...
				if err != nil {
					LogError(err.Error(), ", fileName:"+fileName, ", fieldName:", fieldName, ", index:", r)
					break
				}
			...
			case "int64":
				...
				if err != nil {
					LogError(err.Error(), ", fileName:"+fileName, ", fieldName:", fieldName)
					break
				}
			...
			case "[]int":
				...
				if err != nil {
					...
					if err != nil {
						LogError(err.Error(), ", fileName:"+fileName, ", fieldName:", fieldName, ", row:", r, ", col:", c, ", ", strings.Join(csvData[r], ","))
					} else {
			...
			case "[]int64":
				...
				if err != nil {
					LogError(err.Error(), ", fileName:"+fileName)
					break
				}
				c := reflect.Indirect(data).FieldByName(fieldName)
				newSlice := reflect.Append(c, reflect.ValueOf(v))
			...
			case "[]float34":
				...
				if err != nil {
					...
					c := reflect.Indirect(data).FieldByName(fieldName)
					newSlice := reflect.Append(c, reflect.ValueOf(float64(fv)))
					reflect.Indirect(data).FieldByName(fieldName).Set(newSlice)
				}
			case "[]float64":
				fv, err := strconv.ParseFloat(cellValue, 64)
...
func (self *CsvUtilMgr) LoadCsv(fileName string, SlicePtr interface{}) {
	...
	if len(csvData) <= 1 {
		LogError("len(csvData) <= 1, filename:", fileName)
		os.Exit(1)
		return
	}
...
func (self *CsvUtilMgr) LoadEventsCsv(fileName string, SlicePtr interface{}) {
	...
	csvData := self.readCsv(csvFile)
	if len(csvData) <= 1 {
		LogError("len(csvData/ChapterMap) <= 1, fileName:", fileName)
		os.Exit(1)
		return
	}
